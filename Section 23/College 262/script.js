...
d3
	...
	.append('text')
	.classed('title', true)
	.text('Birth Data of the Year ' + minYear)
	.attr('x', width / 2)
	.attr('y', 30)
	...
	d3
		...
		.on('start', function(d, i) {
			if (i === 0) {
				d3
					.select('.title')
					.text('Updating Data to the Year of ' + year);
			}
		})
		.on('end', function(d, i, nodes) {
			if (i === nodes.length - 1) {
				d3.select('.title').text('Birth Data of the Year ' + year);
			}
		})
		.on('interrupt', function()
	{
		console.log('Interrupted! No longer updating data to the year of ' + year);
	})
		.attr('height', function(d) {
			return height - yScale(d.births);
		})
